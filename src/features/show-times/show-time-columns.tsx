...
type GetShowTimeColumnsParams = {
  onEdit: (row: ShowTimeRow) => void
  onDelete: (row: ShowTimeRow) => void
}

export function getShowTimeColumns({
  onEdit,
  onDelete,
}: GetShowTimeColumnsParams): ColumnDef<ShowTimeTableRow>[] {
  return [
    {
    ...
    },
    {
      ...
      cell: ({ row }) =>
        row.original.startTimeRowSpan > 0 ? (
          <ShowTimeRowActionsMenu
